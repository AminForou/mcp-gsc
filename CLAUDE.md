...
3. Run the API call through `await _execute(...)`, never `.execute()` directly — `google-api-python-client` is synchronous and a bare `.execute()` inside an async tool blocks the event loop for the whole round trip, serialising every concurrent tool call
...
The exception is a synchronous helper that is itself run in a worker thread —
`_inspect_single_url`, used by `batch_url_inspection`, keeps its blocking
`.execute()` because `asyncio.to_thread` already takes it off the loop.
