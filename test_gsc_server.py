...
class TestEventLoopNotBlocked(unittest.IsolatedAsyncioTestCase):
    ...
    batch_url_inspection is deliberately not covered here — it already runs a
    synchronous helper through asyncio.to_thread and has its own concurrency
    test.

    This fails if a future change puts a bare .execute() back on the loop.
    """
